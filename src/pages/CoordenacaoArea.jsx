...
// ═══════════════════════════════════════════════
//  CHART PRIMITIVES — SVG puro, sem dependências
// ═══════════════════════════════════════════════

/** Donut de 3 segmentos: acima (verde) / na média (neutro) / abaixo (âmbar) */
...
/** Donut de 2 segmentos: usado na consolidação (≥ média final x verificação final) */
function DonutChart({ pass, fail, size = 140 }) {
  ...
  // start offset: 0 = 3 o'clock, rotate -90° via transform
  return (
    <svg width={size} height={size} style={{ flexShrink: 0 }}>
      {/* bg track */}
      <circle cx={cx} cy={cy} r={r} fill="none" stroke="var(--border)" strokeWidth={18} />
...
/** Barras horizontais com label, valor e barra colorida */
...
/** Barras verticais agrupadas por pelotão, 3 séries (VC1, VC2, VC3) */
function VGroupedBar({ groups, series, height = 120 }) {
  // groups: [{ label, values: [v1, v2, v3] }]
  ...
  return (
    <ChartSection title="Painel Gerencial — Avaliações">
      ...
      <div className="kpi-grid">
        ...
        <KpiCard label="Acima da Média" value={acima} sub={`${total > 0 ? Math.round((acima / total) * 100) : 0}%`} accent="var(--success)" />
        <KpiCard label="Na Média" value={naMedia} sub={`${total > 0 ? Math.round((naMedia / total) * 100) : 0}%`} accent="#7890a8" />
        <KpiCard label="Abaixo da Média" value={abaixo} sub={`${total > 0 ? Math.round((abaixo / total) * 100) : 0}%`} accent="#E0A800" />
        <KpiCard label="Média Geral" value={mediaGeral !== null ? mediaGeral.toFixed(2) : '—'} accent="var(--gold)" />
      </div>
...
function ConsolidacaoCharts({ visiveis }) {
  ...
  const mediaFinals = visiveis
    .map((a) => a.consolidacao.mediaFinal)
    .filter((v) => v !== null)
  const mediaGeral = avg(mediaFinals)

  // Grupos por pelotão para gráfico de colunas VC1/VC2/VC3
  ...
  // Status por pelotão — barras horizontais de % aptos
  ...
  const VC_SERIES = [
    { label: 'VC1 (Escadas+Poços)', color: 'var(--gold)' },
  ...
  return (
    <ChartSection title="Painel Gerencial — Consolidação">
      ...
      <div className="kpi-grid">
        <KpiCard label="Total" value={total} accent="var(--gold)" />
        <KpiCard label="≥ Média Final" value={aptos} sub={`${total > 0 ? Math.round((aptos / total) * 100) : 0}%`} accent="var(--success)" />
        <KpiCard label="Verificação Final" value={naoAptos} sub={`${total > 0 ? Math.round((naoAptos / total) * 100) : 0}%`} accent="#E55A1E" />
        <KpiCard label="Em Progresso" value={emProgresso} accent="#5b7fff" />
        <KpiCard label="Média Final" value={mediaGeral !== null ? mediaGeral.toFixed(2) : '—'} accent="var(--gold)" />
      </div>
      ...
      <div className="chart-row-2col">
        ...
        <div className="chart-card">
          ...
          <div className="donut-wrapper">
            ...
            <div className="donut-legend">
              <div className="donut-legend-item">
                <span className="donut-dot" style={{ background: 'var(--success)' }} />
                <span>≥ Média Final <strong>{aptos}</strong></span>
              </div>
              <div className="donut-legend-item">
                <span className="donut-dot" style={{ background: '#E55A1E', opacity: 0.85 }} />
                <span>Verificação Final <strong>{naoAptos}</strong></span>
              </div>
              {emProgresso > 0 && (
                <div className="donut-legend-item">
        ...
        {/* % ≥ média final por pelotão */}
        {statusPorPelotao.length > 0 && (
          <div className="chart-card chart-card--grow">
            <p className="chart-card-title">≥ Média Final por Pelotão</p>
            <HBarChart
...
function ExportButtons({ data, columns, filenameBase, disabled }) {
  return (
    <div className="coord-export-group">
      ...
      <button
        ...
        onClick={() => exportToCSV(data, columns, filenameBase)}
      >
      ...
      <button
        ...
        disabled={disabled || data.length === 0}
        onClick={() => exportToXLSX(data, columns, filenameBase)}
      >
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
          <polyline points="14 2 14 8 20 8" />
...
function AvaliacoesTab({ avaliacoes, loading, error }) {
  ...
  const exportData = visiveis.map((av) => ({
    nome: av.studentData.nome || '',
    ordem: av.studentData.ordem || '',
    pelotao: av.studentData.pelotao || '',
    avaliador: av.studentData.avaliador || '',
    data: av.studentData.data || '',
    notaFinal: typeof av.finalScore === 'number' ? av.finalScore.toFixed(2) : '',
    resultado: labelIndividual(getStatusNotaIndividual(Number(av.finalScore || 0))),
    modulo: MODULE_LABELS[av.moduleId] || av.moduleId || '',
  }))

  const filenameBase = `avaliacoes_${filtroModulo === 'all' ? 'todos' : filtroModulo}_${new Date().toISOString().slice(0, 10)}`
  ...
  return (
    <>
      ...
      <div className="coord-filters-card">
        <FilterRow>
          ...
          <div className="coord-filter-field">
            ...
            <select className="coord-filter-select" value={filtroModulo} onChange={(e) => setFiltroModulo(e.target.value)}>
          ...
          <div className="coord-filter-field">
            ...
            <select className="coord-filter-select" value={filtroResultado} onChange={(e) => setFiltroResultado(e.target.value)}>
        ...
        <div className="coord-filter-actions">
          ...
          <ExportButtons data={exportData} columns={exportColumns} filenameBase={filenameBase} disabled={loading} />
        </div>
      </div>
      ...
      {!loading && !error && (
        <>
          {/* Gráficos integrados com filtros */}
...
function ConsolidacaoTab({ consolidacoes, loading, error }) {
  ...
  const exportData = visiveis.map((aluno) => {
    const { vc1, vc2, vc3, mediaFinal, apto } = aluno.consolidacao
    return {
      nome: aluno.nome || '',
      ordem: aluno.ordem || '',
      pelotao: aluno.pelotao || '',
      vc1: vc1 != null ? Number(vc1).toFixed(2) : '',
      vc2: vc2 != null ? Number(vc2).toFixed(2) : '',
      vc3: vc3 != null ? Number(vc3).toFixed(2) : '',
      mediaFinal: mediaFinal != null ? Number(mediaFinal).toFixed(2) : '',
      status: apto === true ? 'APTO' : apto === false ? 'NÃO APTO' : 'EM PROGRESSO',
    }
  })

  const filenameBase = `consolidacao_${new Date().toISOString().slice(0, 10)}`
      ...
      <div className="coord-filters-card">
        <FilterRow>
          ...
          <div className="coord-filter-field">
            ...
            <select className="coord-filter-select" value={filtroStatus} onChange={(e) => setFiltroStatus(e.target.value)}>
  ...
  return (
    <PortalLayout>
      <div style={{ marginBottom: '24px' }}>
        <p className="page-section-label">Área de Coordenação</p>
