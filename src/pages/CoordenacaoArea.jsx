...
/** Donut de 2 segmentos: aprovados (verde) x reprovados (vermelho) */
  ...
  return (
    <ChartSection title="Painel Gerencial — Avaliações">
      ...
      <div className="kpi-grid">
        ...
        <KpiCard label="Aprovados" value={pass} sub={`${total > 0 ? Math.round((pass / total) * 100) : 0}%`} accent="var(--success)" />
        <KpiCard label="Reprovados" value={fail} sub={`${total > 0 ? Math.round((fail / total) * 100) : 0}%`} accent="var(--danger)" />
...
function ConsolidacaoCharts({ visiveis }) {
  ...
  return (
    <ChartSection title="Painel Gerencial — Consolidação">
      ...
      <div className="kpi-grid">
        <KpiCard label="Total" value={total} accent="var(--gold)" />
        <KpiCard label="Aptos" value={aptos} sub={`${total > 0 ? Math.round((aptos / total) * 100) : 0}%`} accent="var(--success)" />
        <KpiCard label="Não Aptos" value={naoAptos} sub={`${total > 0 ? Math.round((naoAptos / total) * 100) : 0}%`} accent="var(--danger)" />
...
function AvaliacoesTab({ avaliacoes, loading, error }) {
  ...
  const exportData = visiveis.map((av) => ({
    ...
    resultado: av.isPassing ? 'APROVADO' : 'REPROVADO',
                    ...
                    return (
                      <tr key={aluno.ordem}>
                        ...
                        <td className="center">
                          <VcTooltip
                            label="VC1"
                            lines={[fmtMod('escadas', 'Escadas'), fmtMod('pocos', 'Poços')]}
                          >
                            {fmtNota(vc1)}
                          </VcTooltip>
                        </td>
