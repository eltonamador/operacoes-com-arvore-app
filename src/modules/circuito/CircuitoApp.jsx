...
export default function CircuitoApp() {
  ...
  async function loadEvaluations() {
    try {
      ...
      const evaluations = await fetchAvaliacoesByModulo('circuito')
  ...
  async function saveEvaluation(dadosAvaliacao) {
    const saved = await saveAvaliacao({ ...dadosAvaliacao, module_id: 'circuito' })
  ...
  return (
    <div className="app-root">
      ...
      {state.screen === 'form' && (
        <StudentForm
          {...props}
          moduleName="Circuito Operacional"
          moduleEmoji="🔄"
        />
      )}
